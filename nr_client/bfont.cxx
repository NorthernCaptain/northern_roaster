...
void InitFont(BFont_Info *Font)
{
    ...
    Font->Chars[32].x = 0;
    Font->Chars[32].y = 0;
    Font->Chars[32].h = Font->Surface->h;
    Font->Chars[32].w = Font->Chars[int('p')].w;
...
int PutStringFont(SDL_Surface *Surface, BFont_Info *Font, int x, int y, const char *text)
{
    int orig_x=x;
    int i=0;
    while (text[i]!='\0') {
        x  += PutCharFont(Surface,Font,x,y,int((unsigned)(text[i]) & 0xff));
        i++;
    }
    return x - orig_x;
}
...
int TextWidthFont(BFont_Info *Font, const char *text)
{
    ...
    while (text[i]!='\0') {
            x += CharWidth(Font,(unsigned)(text[i]) & 0xff);
            i++;
    }
    return x;
}
