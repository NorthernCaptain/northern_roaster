...
template < class TC > class bilist
{
 ...
 public:
  ...
  inline bool is_last() {return next==0;};
  inline bool is_first() {return prev==0;};

  inline bilist<TC> *get_next() { return next;};
  inline bilist<TC> *get_prev() { return prev;};

  void set_prev(bilist<TC> *plist) { prev=plist;};
  void set_next(bilist<TC> *plist) { next=plist;};

  inline TC& get_object() { return object;};
};
