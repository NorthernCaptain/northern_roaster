...
void set_params(SerialStream& dev)
{
    ...
    dev.SetFlowControl( SerialStreamBuf::FLOW_CONTROL_HARD ) ;
    dev.SetVMin(0);
    dev.SetVTime(20);
}


int read_channels(SerialStream& dev, short int *ibuf)
{
    std::string dev_address = "0001";
    std::string snd = "$";
    snd += dev_address + "RR000010";
    ...
    {
	dev >> buf[i];
	DBG(5, "CHAR: " << DHEX(buf[i]) << " : " << buf[i]);
	if(buf[i] == 0x0d)
	{
	    buf[i]=0;

	    snd = get_checksum(buf, i-2);
	    if(buf[i-2] != snd[0] || buf[i-1]!=snd[1])
	    {
		DBG(5, "IRT::read_channels: WRONG checksum: " << (buf + i -2) 
		    << " need: " << snd);
		return 0;
	    }
